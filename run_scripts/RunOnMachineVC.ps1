...
write-output "Using output file $outputFile"
...
Invoke-Command -ComputerName $computer -Credential $cred -Authentication CredSSP -ScriptBlock { 
    ...
    if ([System.IO.Path]::GetExtension($using:code) -ne ".py") {
        ...
        $out = invoke-expression "& '$using:code' '$using:filename' '$guid'" -ErrorVariable errout 
    ...
    stop-job $monitorJob
    $outmon = Receive-Job -Job $monitorJob
...
log "running dev verion - exitCode = $exitcode"
