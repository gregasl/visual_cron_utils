param(
    ...
    #$code ="\\aslfile01\aslcap\IT\software\development\ION\ION_Static_Data.bat",
	$filename="",
    $jobID=$null
)
...
#             and the parameter will be handed into the command
...
Function Log {
    ...
    # the logging to a file causes the file to be in use so prams wrongly fail. Commenting out until we have a better way. No needed really with VisualCron
    #try { 
...
if ($code[1] -ne ":") {
    ...
    $finalFile = "$outputDir\Output\" + $jn + $fileDate + ".txt"
}
$outputDir = 'c:\temp'
$outputFile = "$outputDir\" + $jn + $fileDate + ".txt"

write-output "Using output file $outputFile - final file is $finalFile"
...
Invoke-Command -ComputerName $computer -Credential $cred -Authentication CredSSP -ScriptBlock { 
    ...
    if (Test-Path "C:\Program Files\Python39\python.exe"  ) {
        if ($parts[1] -eq 108) {
            $execPython = "\\aslhost02\Python\python.exe"
        } else {
            $execPython = "\\aslhost02\Python\python.exe"
        }
...
Log "Before final file copy"
if ($finalFile -ne "") {
    write-output "Copying $outputFile to $finalFile"
    Copy-Item -Path $outputFile -Destination $finalFile -Force
